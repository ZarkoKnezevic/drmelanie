...
interface BabyGrowthProps {
  blok: StoryblokBlok & {
    eight_weeks_text?: string;
    twelve_weeks_text?: string;
    twenty_weeks_text?: string;
    twenty_four_weeks_text?: string;
    twenty_eight_weeks_text?: string;
...
const WEEKS = [
  { value: 0, label: '8', progress: 0 },
  { value: 1, label: '12', progress: 28 },
  { value: 2, label: '20', progress: 84 },
  { value: 3, label: '24', progress: 140 },
  { value: 4, label: '28', progress: 168 },
  { value: 5, label: '32', progress: 196 },
  { value: 6, label: '36', progress: 224 },
  { value: 7, label: '40', progress: 252 },
] as const;

const WEEK_TEXT_FIELDS = [
  'eight_weeks_text',
  'twelve_weeks_text',
  'twenty_weeks_text',
  'twenty_four_weeks_text',
  'twenty_eight_weeks_text',
  'thirty_two_weeks_text',
  'thirty_six_weeks_text',
  'forty_weeks_text',
] as const;
...
export default function BabyGrowth({ blok }: BabyGrowthProps) {
  ...
  useEffect(() => {
    ...
    const handleProgressChange = (progress: number) => {
      const percentage = progress / 252;
      const animations = svgRef.current?.querySelectorAll('animate');

      animations?.forEach((animation) => {
        const duration = parseFloat(animation.getAttribute('dur') || '100');
        let targetTime = duration * percentage;
        if (targetTime <= 0) targetTime = 0.01;
        if (targetTime >= duration) targetTime = duration - 0.1;
    ...
    >
      <div className="spacing container">
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-2 lg:gap-12">
          {/* Left Column: Animation with Sticky Buttons */}
          <div className="relative flex min-h-[600px] flex-col lg:sticky lg:top-24 lg:h-[calc(100vh-12rem)]">
            {/* SVG Animation */}
            ...
            {/* Sticky Week Buttons */}
            <div className="absolute left-0 top-1/2 z-10 flex -translate-y-1/2 flex-col gap-2 lg:left-4">
              {WEEKS.map((week, index) => (
                ...
                >
                  ...
                  <span className="text-xs font-normal leading-tight">weeks</span>
                </button>
              ))}
            </div>
          </div>
